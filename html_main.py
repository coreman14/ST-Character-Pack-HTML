...
def create_html(args, yml):
    ...
    for count, character_name in enumerate(
        ...
        for pose_path in [
            ...
            widths = []
            heights = []
            for sizes in map(trim_images, faces):
                widths.append(sizes[0])
                heights.append(sizes[1])
            ...
            widths.clear()
            heights.clear()
            for sizes in map(trim_images, outfits):
                widths.append(sizes[0])
                heights.append(sizes[1])
